    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-2">
                @if($isAdmin)
                <a href="{{ route('savings-accounts.settings') }}" class="px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white text-sm font-medium rounded-lg transition">
                    ⚙️ Paramètres
                </a>
                @endif
                <a href="{{ route('savings-accounts.create') }}" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-lg transition">
                    + Ouvrir un compte
                </a>
            </div>
        </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                    ...
                    <p class="text-3xl font-bold text-emerald-600 dark:text-emerald-400 mt-2">{{ number_format($activeAccounts) }}</p>
                </div>
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    ...
                    <form method="GET" class="flex flex-wrap gap-4 items-end">
                        ...
                        @endif
                        <button type="submit" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm">Rechercher</button>
                        @if(request()->hasAny(['search','status']))
                        <a href="{{ route('savings-accounts.index') }}" class="px-4 py-2 bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200 rounded-lg text-sm">Réinitialiser</a>
                        @endif
                    </form>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-12 text-center">
                    ...
                    <p class="text-gray-500 dark:text-gray-400 text-sm">Utilisez la recherche ci-dessus pour trouver un compte épargne.</p>
                    <a href="{{ route('savings-accounts.create') }}" class="mt-4 inline-block px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-medium">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                <td class="px-6 py-4 whitespace-nowrap font-mono text-sm text-gray-900 dark:text-gray-100">{{ $account->account_number }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $account->client?->full_name ?? '—' }}</div>
                                    <div class="text-xs text-gray-500">{{ $account->client?->phone }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900 dark:text-gray-100">{{ number_format($account->balance, 2) }} HTG</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @php
                                    ...
                                    @endphp
                                    <span class="px-2 py-1 rounded-full text-xs font-medium {{ $badge }}">{{ ucfirst($account->status) }}</span>
                                </td>
                                @if($isAdmin)
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $account->created_at->format('d/m/Y') }}</td>
                                @endif
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex gap-3">
                                        <a href="{{ route('savings-accounts.show', $account) }}" class="text-blue-600 hover:text-blue-800 dark:text-blue-400">Voir</a>
                                        @if($account->status === 'actif')
                                        <a href="{{ route('savings-accounts.deposit.form', $account) }}" class="text-green-600 hover:text-green-800 dark:text-green-400">Dépôt</a>
                                        <a href="{{ route('savings-accounts.withdraw.form', $account) }}" class="text-orange-600 hover:text-orange-800 dark:text-orange-400">Retrait</a>
                                        @endif
                                    </div>
                                </td>
                            </tr>
                ...
                <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700">{{ $accounts->links() }}</div>
                @endif
            </div>
            @endif
